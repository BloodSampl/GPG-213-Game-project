...
namespace aitcHUtils.MobileTouch
{
	public class DetectTouchMovement : MonoBehaviour
	{
		...
		/// <summary>
		///  The amount that can be used as zoom
		/// </summary>
		static public float zoomAmount;
		...
		public void Calculate()
		{
			...
			zoomAmount = turnAngle = turnAngleDelta = 0;
			...
			if (Input.touchCount == 2)
			{
				...
				if (touch1.phase == TouchPhase.Moved || touch2.phase == TouchPhase.Moved)
				{
					...
					pinchDistanceDelta = currentMagnitude - prevMagnitude;

                    if (Mathf.Abs(pinchDistanceDelta) > minPinchDistance)
                    {
						zoomAmount = pinchDistanceDelta * pinchRatio;
					}
                    else
                    {
                        zoomAmount = pinchDistance = pinchDistanceDelta = 0;
                    }
					#endregion
